...
| 場次管理 | `/sessions` | 顯示場次列表，可新增/刪除場次 |
| 排行榜 | `/stats` | 顯示深青 hero + 排行榜 + 徽章 |
| 管理員啟用 | `/?token=your-secret-token` | 頂部出現青色管理員橫幅，TabBar 出現第 4 個「管理」頁籤 |
| 編輯場次 | 管理員模式下，於 `/sessions` 點擊場次的「編輯」 | 彈出 inline 編輯 modal，可勾選成員、搜尋訪客 |
| 成員管理 | `/members`（管理員模式） | 可停用/啟用成員、新增成員、將訪客升級為正式成員 |
| 儲存名單 | 編輯 modal 勾選成員 → 儲存 | 畫面更新 + Google Sheets 資料寫入 |
...
```
https://your-vercel-url/?token=your-secret-token
```

啟用後 token 會自動儲存在瀏覽器 localStorage，之後不需再帶 token 即可維持管理員身份（同一瀏覽器、同一裝置）。

**編輯出席名單：**
1. 進入 `/sessions` 頁面
2. 點擊場次卡片的「編輯」按鈕（或在 `/` 首頁點右下角鉛筆 FAB）
3. 在 inline 編輯 modal 中勾選出席成員
4. 在「臨時成員」區塊用模糊搜尋選擇曾出席的訪客，或直接輸入新訪客姓名
5. 點「儲存並發布」

**登出管理員模式：**
點擊頂部青色管理員橫幅右側的「登出管理」按鈕。
...
在管理員模式下，進入 `/members`（TabBar 第 4 個「管理」頁籤）：

- **新增成員**：在「新增成員」區塊輸入姓名後點「新增」
- **停用成員**：在「活躍成員」區塊點成員右側的「停用」按鈕（歷史記錄保留）
- **重新啟用**：在「已停用成員」區塊點「啟用」按鈕
- **訪客升級為正式成員**：在「可升級訪客」區塊點「升級」，系統會自動建立成員並回溯更新歷史出席記錄
...
**Q：換了瀏覽器或裝置後管理員模式失效？**
Token 儲存在瀏覽器 localStorage，更換瀏覽器或裝置需重新用 `?token=xxx` 網址啟用。

**Q：舊的 `/admin` 網址還能用嗎？**
`/admin` 會自動重新導向到 `/sessions`，並保留 `?date=` 等 query 參數。
